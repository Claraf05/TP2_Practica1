...
public class Controller {
	...
	public void run(int n, OutputStream out, InputStream expOut, StateComparator cmp) {
		if(expOut != null) {
			...
			JSONArray listaStates = new JSONArray();
			listaStates = j.getJSONArray("states");
	...
	public void loadBodies(InputStream in) {
		...
		JSONArray listaStates = new JSONArray();
		listaStates = j.getJSONArray("bodies");
		List<JSONObject> lista = new ArrayList<JSONObject>();
		...
		for(int k = 0; k< listaStates.length(); k++) {
			...
			ps.addBody(f.createInstance(listaStates.getJSONObject(k)));
		}
	}
}
