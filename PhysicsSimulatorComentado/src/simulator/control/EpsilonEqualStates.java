...
public class EpsilonEqualStates implements StateComparator{
	...
	public boolean equal(JSONObject s1, JSONObject s2) { //compara 2 estados con el metodo que usa epsilon
		...
		if(s1.get("time") == s2.get("time")) { //si el tiempo actual de ambos es igual
			if(s1.getJSONArray("bodies").length() == s2.getJSONArray("bodies").length()) { //si ambos estados tienen el mismo numero de bodies
				for(int i = 0; i< s1.getJSONArray("bodies").length(); i++) { //recorre la lista
					...
					if(obj1.get("id") == obj2.get("id")) { //si tienen el mismo id los 2 estados a comparar
						...
						if(Math.abs(obj1.getDouble("m") - obj2.getDouble("m")) <= this.eps) {
							...
							if((v1.distanceTo(v2)) <= this.eps){ //si la diferencia de posiciones entre los cuerpos es menor que epsilon
								...
								if((v1.distanceTo(v2)) <= this.eps){ //si la diferencia de velocidades entre los cuerpos es menor que epsilon
									...
									if((v1.distanceTo(v2)) <= this.eps){ //si la diferencia de fuerzas entre los cuerpos es menor que epsilon
